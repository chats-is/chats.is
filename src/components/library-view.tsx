...
function CardActions({
  ...
  return (
    <div className="absolute top-2 right-2 z-10 flex items-center gap-1">
      ...
      {chatId && (
        <Link
          to="/chat/$chatId" params={{ chatId: chatId }}
          title="Open chat"
          className={buttonClass}
        >
    ...
    return (
      <div className="group relative aspect-square">
        <div
          ...
          onClick={() => item.chatId && router.navigate({ to: '/chat/$chatId', params: { chatId: item.chatId } })}
          onKeyDown={event => {
            if (event.key === 'Enter' || event.key === ' ') {
              event.preventDefault();
              if (item.chatId) router.navigate({ to: '/chat/$chatId', params: { chatId: item.chatId } });
            }
          }}
        >
